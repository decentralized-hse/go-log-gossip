...
func NewGossipStore(id string, version, clusterId, selfClusterDomain string) *GossipStoreImpl {
	n := &GossipStoreImpl{}
...
func (s *GossipStoreImpl) NodeId() string {
	return s.id
}
...
func (s *GossipStoreImpl) UpdateNodeStatus(nodeId string, status types.NodeStatus) error {
	s.Lock()
	defer s.Unlock()
